...
function* action_access_save(event) {
	...
	let stored = (yield* StorageInternal("sync").getStorageValue(null)) || {}
	let success = true
	...
	if (success) {
		yield* StorageInternal("sync").setStorageValue(stored)
...
function* exchange_permissions_check() {
	...
	getState(this).permissions.granted = (hasPermission && granted)
...
function* subscription_init() {
	...
	const packages = subscriptionContainer.querySelectorAll("table thead .panel")
	const rows = subscriptionContainer.querySelectorAll(".panel table tbody tr")
