...
function Bitfinex() {
	...
	function* positionsCloseAll(Command) {
		...
		for (let i = 0; i < positions.length; i++) {
			...
			let quantity = Command.q.reset()
			if (quantity.getIsPercent()) {
				quantity.mul(amount)
			}

			let order = {}
			order.symbol = position.symbol
			order.amount = quantity.resolve(8)
			order.price = price
			order.side = side
			switch (Command.t) {
				case "fok":
	...
	function* symbolInfo(symbol) {
		...
		if (!window.BITFINEX_SYMBOLS || window.BITFINEX_SYMBOLS._updated < expired) {
			...
			window.BITFINEX_SYMBOLS._updated = Date.now()
		}
