...
function _1Broker() {
	...
	function* get(resource, query) {
		...
		if (response.error) {
			throw new Error(response.error_message || response.error_code)
		}
	...
	function* positionsClose(Command, position) {
		...
		if (Command.sl || Command.hasOwnProperty("tp") || Command.hasOwnProperty("ts")) {
			if (Command.sl) {
				query.stop_loss = Command.sl.relative(position.entry_price).resolve(8)
			}
			if (Command.hasOwnProperty("tp")) {
				query.take_profit = Command.tp.compare(0) ? Command.tp.relative(position.entry_price).resolve(8) : "null"
			}
			if (Command.hasOwnProperty("ts")) {
				if (!Command.sl) {
					if (position.stop_loss) {
						query.stop_loss = position.stop_loss // Use existing when only changing Trailing Stop Loss
					} else {
						throw new SyntaxError("Stop Loss is required to place a Trailing Stop Loss.")
					}
				}
				query.trailing_stop_loss = Command.ts.compare(0) ? "true" : "false"
			}
		} else if (Command.t !== "market") {
			throw new SyntaxError("Empty position close. Market close or use Stop Loss and/or Take Profit.")
		}

		if (Command.d) {
			console.info(this.getExchangeName(), query)
			return false // Disabled
		}
